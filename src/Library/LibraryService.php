...
final class LibraryService
{
    ...
    public function lendBook(string $bookId, string $memberId, string $todayYmd): string
    {
        ...
        if ($member === null) {
            return "Errore: membro non trovato (id=$memberId). Suggerimento: usa members:list";
        }
        ...
        if ($openLoans >= $this->maxLoansPerMember) {
            return "Errore: limite prestiti raggiunto per il membro $memberId. Prestiti aperti: openLoans=$openLoans, limite consentito: limit={$this->maxLoansPerMember}. Restituisci un libro prima di richiederne un altro.";
        }
    ...
    public function bookStatus(string $bookId): string
    {
        ...
        // Se il libro è prestato, mostra info del prestito
        if (!$book->isAvailable()) {
            $openLoan = $this->loans->findOpenLoanByBookId($bookId);
            if ($openLoan !== null) {
                $output .= sprintf(
